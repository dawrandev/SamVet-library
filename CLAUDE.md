...
- **Katta fayl yuklash (chunked upload):** 20MB'dan katta fayllar (hozircha Kitob PDF + Video trek) kichik bo'laklarga bo'lib ketma-ket yuklanadi — `App\Services\ChunkedUploadService`, `app/Enums/ChunkedUploadKind.php`, admin JS `resources/js/admin/upload-form.js`. Sabab: production hosting'da root/sudo yo'q, server yuklash limitlarini bilmaymiz/o'zgartira olmaymiz — kichik bo'laklar bu limitga umuman urilmaydi. Yig'ilgan fayl **haqiqiy kontent bo'yicha** qayta tekshiriladi (`mimes:`/`extensions:` qoidasi, mijoz aytgan MIME'ga emas), token bir martalik (`claim()` sessiyani o'chiradi), tashlab ketilgan sessiyalar cron kerak bo'lmasdan lazy tozalanadi. Qolgan 5 ta katta-fayl joyi (Dissertatsiya, Avtoreferat, Maqola, Audio trek, Jurnal soni) hali eski to'g'ridan-to'g'ri yo'lda — xuddi shu naqsh keyingi bosqichda takrorlanadi.
