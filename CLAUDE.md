...
- **Aqilli qidiruv (client sayt — katalog):** real-time, imlo xatoni kechiradigan, relevantlik bo'yicha — **MySQL FULLTEXT** asosida amalga oshirilgan (`CatalogRepository::applySmartSearch()`), server root/sudo huquqi yo'qligi sababli Meilisearch o'rniga: `BOOLEAN MODE` + prefiks so'rov (`so'z*`) — typo/qisqartirish tolerantligi; sarlavha uchun qo'shimcha boost (`CASE WHEN ... LIKE ... THEN 10`) — relevantlik; 3 belgidan qisqa so'zlar (`innodb_ft_min_token_size` server config talab qiladi, bizda kirish yo'q) — so'z chegarasi taxmin qilingan `LIKE` bilan; 0 natija — `SearchSuggestionService` (PHP Levenshtein) orqali eng yaqin so'z topiladi va o'sha so'z bo'yicha natijalar avtomatik ko'rsatiladi ("«X» uchun natija topilmadi, «Y» bo'yicha ko'rsatilmoqda" banneri bilan — faqat bosiladigan havola emas), xuddi shu tolerantlik `catalog.quick-search` tezkor qidiruvida ham ishlaydi; tezkor (typeahead) natijalar — `catalog.quick-search` JSON endpoint + Alpine `fetch` (`resources/js/site/search-typeahead.js`). Qidiruv butunlay `CatalogRepository` qatlamida izolyatsiya qilingan — server imkoniyati kengaysa (root/sudo bo'lsa), Meilisearch'ga o'tish shu bitta qatlamni almashtirish bilan cheklanadi, Controller/Service/Blade/DTO'larga tegilmaydi. Admin panel qidiruvi va Jurnal/Maqola qidiruvi hali oddiy `LIKE` — bu qamrovga kirmagan.
