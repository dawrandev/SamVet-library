...
it('sums book price by acquisition year, ignoring copies with no date or books with no price', function () {
    $priced = Book::factory()->create(['price' => 30000]);
    BookCopy::factory()->create(['book_id' => $priced->id, 'acquisition_act_at' => '2024-03-01']);
    BookCopy::factory()->create(['book_id' => $priced->id, 'acquisition_act_at' => '2024-09-01']);

    $unpriced = Book::factory()->create(['price' => null]);
    BookCopy::factory()->create(['book_id' => $unpriced->id, 'acquisition_act_at' => '2024-05-01']);

    $noDate = Book::factory()->create(['price' => 99999]);
    BookCopy::factory()->create(['book_id' => $noDate->id, 'acquisition_act_at' => null]);
    ...
    expect($yearIdx)->not->toBeFalse()
        ->and($report['books'][$yearIdx])->toBe(60000.0) // 2 copies x 30000, unpriced/undated books contribute 0
...
it('zero-fills every year across the combined book and subscription range', function () {
    ...
    BookCopy::factory()->create(['book_id' => $book->id, 'acquisition_act_at' => '2022-01-01']);
