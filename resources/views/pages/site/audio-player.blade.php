...
@section('content')
    <div class="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8"
         x-data="{
             current: {{ $audiobook->tracks->first()?->id ?? 'null' }},
             tracks: @js($audiobook->tracks->map(fn ($t) => ['id' => $t->id, 'title' => $t->title, 'url' => route('listen.audiobook.file', [$audiobook->slug, $t->id])])),
    ...
    >
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                <p class="truncate text-base font-bold text-gray-900">{{ $audiobook->name }}</p>
                @if ($audiobook->author)
                    <p class="truncate text-xs text-gray-500">{{ $audiobook->author }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="mt-5 overflow-hidden rounded-2xl border border-gray-200 bg-white">
            <h2 class="border-b border-gray-100 px-5 py-3.5 text-sm font-bold text-gray-900">{{ __('Audiolar') }}</h2>
        ...
        <p class="mt-3 text-center text-xs text-gray-400">{{ __('Faqat online tinglash · Yuklab olish mavjud emas') }}</p>
    </div>
@endsection
