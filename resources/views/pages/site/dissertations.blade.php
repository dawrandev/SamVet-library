...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav class="text-sm text-gray-500">
            ...
            <span class="mx-1.5 text-gray-300">/</span>
            <span class="text-gray-700">{{ __('Dissertatsiyalar') }}</span>
        </nav>

        <div class="mt-3 flex flex-wrap items-end justify-between gap-3">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">{{ __('Dissertatsiyalar') }}</h1>
            ...
            <span class="rounded-lg bg-blue-50 px-3 py-1.5 text-sm font-semibold text-blue-700">
                {{ __(':n ta dissertatsiya', ['n' => number_format($dissertations->total(), 0, '.', ' ')]) }}
            </span>
        </div>

        {{-- Search --}}
        <form method="GET" action="{{ route('dissertations.index') }}" class="mt-6 flex max-w-md gap-2">
        ...
        @if ($dissertations->isEmpty())
            <div class="mt-8 rounded-2xl border border-dashed border-gray-300 bg-white p-12 text-center">
                <p class="text-sm font-semibold text-gray-900">{{ __('Hozircha dissertatsiyalar yo‘q') }}</p>
                <p class="mt-1 text-sm text-gray-500">{{ __('Dissertatsiyalar qo‘shilgach shu yerda ko‘rinadi.') }}</p>
            </div>
        @else
            <div class="mt-7 divide-y divide-gray-100 overflow-hidden rounded-2xl border border-gray-200 bg-white">
                @foreach ($dissertations as $dissertation)
                    <a href="{{ route('dissertation.show', $dissertation->slug) }}"
                       class="flex items-center gap-4 px-5 py-4 transition hover:bg-gray-50">
                        <div class="min-w-0 flex-1">
                            <p class="line-clamp-2 text-sm font-semibold text-gray-900">{{ $dissertation->title }}</p>
                            <p class="mt-1 text-xs text-gray-500">
                                {{ $dissertation->author ?: '—' }}
                                @if ($dissertation->degree) · {{ $dissertation->degree->label() }} @endif
                                @if ($dissertation->defense_year) · {{ $dissertation->defense_year }} @endif
                            </p>
                        </div>
                        @if (filled($dissertation->electronic_file))
            ...
            <div class="mt-10">
                {{ $dissertations->onEachSide(1)->links() }}
            </div>
        @endif
    </div>
@endsection
