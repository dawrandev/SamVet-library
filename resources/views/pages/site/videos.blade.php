...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav class="text-sm text-gray-500">
            ...
            <span class="mx-1.5 text-gray-300">/</span>
            <span class="text-gray-700">{{ __('Videolar') }}</span>
        </nav>

        <div class="mt-3 flex flex-wrap items-end justify-between gap-3">
            <div>
                <h1 class="text-3xl font-extrabold tracking-tight text-gray-900">{{ __('Videolar') }}</h1>
                <p class="mt-1.5 text-sm text-gray-500">{{ __('Tomosha qilish uchun tizimga kirish talab qilinadi.') }}</p>
            </div>
            <span class="rounded-lg bg-blue-50 px-3 py-1.5 text-sm font-semibold text-blue-700">
                {{ __(':n ta video', ['n' => number_format($videos->total(), 0, '.', ' ')]) }}
            </span>
        </div>

        {{-- Search --}}
        <form method="GET" action="{{ route('videos.index') }}" class="mt-6 flex max-w-md gap-2">
