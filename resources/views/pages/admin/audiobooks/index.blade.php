...
@section('title', __('Audiolar'))

@section('content')
    {{-- Header + New audiobook --}}
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800 dark:text-white/90">{{ __('Audiolar') }}</h2>
            <p class="text-theme-sm mt-1 text-gray-500 dark:text-gray-400">{{ __('Jami') }}: {{ $audiobooks->total() }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.audiobooks.export', $filters) }}"
            ...
            <a href="{{ route('admin.audiobooks.create') }}"
               class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition">
                <span class="text-lg leading-none">+</span> {{ __('Yangi audiokitob') }}
            </a>
        </div>
    </div>
    ...
    {{-- Search --}}
    <form method="GET" action="{{ route('admin.audiobooks.index') }}"
        ...
        <div class="flex gap-2">
            ...
            @if (array_filter($filters))
                <a href="{{ route('admin.audiobooks.index') }}" class="flex h-11 items-center rounded-lg border border-gray-200 px-4 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-400">{{ __('Tozalash') }}</a>
            @endif
        </div>
    </form>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-800">
                        <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Audiokitob') }}</th>
                        <th class="px-5 py-3 text-left text-theme-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Audiolar soni') }}</th>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-14 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-md bg-gray-100 text-lg dark:bg-gray-800">
                                        ...
                                        @else
                                            <x-admin.icon name="speaker-wave" class="h-6 w-6 text-gray-400" />
                                        @endif
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-theme-sm truncate font-medium text-gray-800 dark:text-white/90">{{ $audiobook->name }}</p>
                                        <p class="text-theme-xs truncate text-gray-500 dark:text-gray-400">{{ $audiobook->author ?: '—' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4 text-theme-sm text-gray-600 dark:text-gray-400">{{ $audiobook->tracks_count }}</td>
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.audiobooks.show', $audiobook) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-5 py-12 text-center">
                                <x-admin.icon name="speaker-wave" class="mx-auto h-10 w-10 text-gray-300 dark:text-gray-600" />
                                <p class="mt-2 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Audiokitoblar topilmadi.') }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
