...
@section('content')
    {{-- Title + New avtoreferat --}}
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800 dark:text-white/90">{{ __('Avtoreferatlar') }}</h2>
            <p class="text-theme-sm mt-1 text-gray-500 dark:text-gray-400">{{ __('Jami') }}: {{ $avtoreferats->total() }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.avtoreferats.create') }}"
               class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition">
                <span class="text-lg leading-none">+</span> {{ __('Yangi avtoreferat') }}
            </a>
        </div>
    </div>
    ...
    {{-- Search / filter --}}
    <form method="GET" action="{{ route('admin.avtoreferats.index') }}"
        ...
        <div class="flex gap-2">
            ...
            @if (array_filter($filters))
                <a href="{{ route('admin.avtoreferats.index') }}" class="flex h-11 items-center rounded-lg border border-gray-200 px-4 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-400">{{ __('Tozalash') }}</a>
            @endif
        </div>
    </form>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="max-w-full overflow-x-auto">
            <table class="min-w-full">
                ...
                <tbody>
                    @forelse ($avtoreferats as $avtoreferat)
                        <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]">
                            <td class="px-5 py-4">
                                <div class="min-w-0">
                                    <p class="text-theme-sm truncate font-medium text-gray-800 dark:text-white/90">{{ $avtoreferat->title }}</p>
                                    <p class="text-theme-xs truncate text-gray-500 dark:text-gray-400">{{ $avtoreferat->author }}</p>
                                </div>
                            </td>
                            <td class="px-5 py-4 text-theme-sm text-gray-600 dark:text-gray-400">
                                {{ $avtoreferat->journalIssue?->journal?->name ?? '—' }}
                                @if ($avtoreferat->journalIssue)
                                    <span class="text-theme-xs text-gray-400">({{ $avtoreferat->journalIssue->issue_number }}, {{ $avtoreferat->journalIssue->year }})</span>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.avtoreferats.show', $avtoreferat) }}"
                                    ...
                                    <a href="{{ route('admin.avtoreferats.edit', $avtoreferat) }}"
