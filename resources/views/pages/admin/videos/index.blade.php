...
@section('title', __('Videolar'))

@section('content')
    {{-- Header + New video --}}
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800 dark:text-white/90">{{ __('Videolar') }}</h2>
            <p class="text-theme-sm mt-1 text-gray-500 dark:text-gray-400">{{ __('Jami') }}: {{ $videos->total() }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.videos.export', $filters) }}"
            ...
            <a href="{{ route('admin.videos.create') }}"
               class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition">
                <span class="text-lg leading-none">+</span> {{ __('Yangi video') }}
            </a>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.videos.index') }}"
        ...
        <div class="flex gap-2">
            ...
            @if (array_filter($filters))
                <a href="{{ route('admin.videos.index') }}" class="flex h-11 items-center rounded-lg border border-gray-200 px-4 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-400">{{ __('Tozalash') }}</a>
            @endif
        </div>
    </form>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-14 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-md bg-gray-100 text-lg dark:bg-gray-800">
                                        ...
                                        @else
                                            <x-admin.icon name="video-camera" class="h-6 w-6 text-gray-400" />
                                        @endif
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-theme-sm truncate font-medium text-gray-800 dark:text-white/90">{{ $video->name }}</p>
                                        <p class="text-theme-xs truncate text-gray-500 dark:text-gray-400">{{ $video->author ?: '—' }}</p>
                                    </div>
                                </div>
                            </td>
