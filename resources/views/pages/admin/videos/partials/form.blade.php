...
<form method="POST"
      action="{{ $video ? route('admin.videos.update', $video) : route('admin.videos.store') }}"
    ...
    <div class="sticky top-16 z-9 -mx-4 mb-6 flex items-center justify-between border-b border-gray-200 bg-gray-50/90 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6 dark:border-gray-800 dark:bg-gray-900/90">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-lg font-bold text-gray-800 dark:text-white/90">
                {{ $video ? __('Videoni tahrirlash') : __('Yangi video') }}
            </h2>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.videos.index') }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 dark:border-gray-800 dark:text-gray-400">{{ __('Bekor qilish') }}</a>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        <x-admin.form.section :title="__('Muqova')">
            <x-admin.form.file name="cover" :image="true" accept="image/*" with-progress
                :currentUrl="$video?->cover_image ? asset('storage/' . $video->cover_image) : null"
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="annotation" id="annotation" rows="6"
                              class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 w-full rounded-lg border bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:bg-gray-900 dark:text-white/90 {{ $errors->has('annotation') ? 'border-error-500' : 'border-gray-300 dark:border-gray-700' }}">{{ old('annotation', $video?->annotation) }}</textarea>
