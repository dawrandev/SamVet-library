...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.videos.index') }}" class="flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-gray-800">&larr;</a>
            <h2 class="text-xl font-bold text-gray-800 dark:text-white/90">{{ $video->name }}</h2>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.videos.edit', $video) }}" class="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 dark:border-gray-800 dark:text-gray-400">{{ __('Tahrirlash') }}</a>
            <button type="button"
                    @click="$store.confirm.ask('{{ route('admin.videos.destroy', $video) }}', '{{ __('Videoni o‘chirishni tasdiqlaysizmi?') }}')"
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
                <div class="mx-auto flex h-56 w-40 items-center justify-center overflow-hidden rounded-xl bg-gray-100 dark:bg-gray-800">
                    @if ($video->cover_image)
                        <img src="{{ asset('storage/' . $video->cover_image) }}" alt="" class="h-full w-full object-cover" />
                    @else
                        <x-admin.icon name="video-camera" class="h-14 w-14 text-gray-300 dark:text-gray-600" />
                    @endif
                </div>

                <dl class="mt-5 space-y-3">
                    <div class="flex justify-between gap-4 border-b border-gray-50 pb-2 dark:border-gray-800/50">
                        <dt class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Video nomi') }}</dt>
                        <dd class="text-theme-sm text-right font-medium text-gray-800 dark:text-white/90">{{ $video->name }}</dd>
                    </div>
                    @if ($video->author)
                        <div class="flex justify-between gap-4 border-b border-gray-50 pb-2 dark:border-gray-800/50">
                            <dt class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Muallifi') }}</dt>
                            <dd class="text-theme-sm text-right font-medium text-gray-800 dark:text-white/90">{{ $video->author }}</dd>
                        </div>
                    @endif
                    <div class="flex justify-between gap-4 border-b border-gray-50 pb-2 dark:border-gray-800/50">
                        <dt class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Joylangan sana') }}</dt>
                        <dd class="text-theme-sm text-right font-medium text-gray-800 dark:text-white/90">{{ $video->published_at?->format('d.m.Y') ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between gap-4 pb-2">
                        <dt class="text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Videolar soni') }}</dt>
                        <dd class="text-theme-sm text-right font-medium text-gray-800 dark:text-white/90">{{ $video->tracks->count() }}</dd>
                    </div>
                </dl>
                ...
                @if ($video->annotation)
                    <div class="mt-5 border-t border-gray-100 pt-4 dark:border-gray-800">
                        ...
                        <p class="text-theme-sm text-gray-500 dark:text-gray-400">{{ $video->annotation }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="col-span-12 space-y-6 xl:col-span-8">
            ...
            >
                <div class="flex items-center justify-between border-b border-gray-100 p-5 dark:border-gray-800 sm:px-6">
                    <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Videolar') }}</h3>
                ...
                <div class="max-w-full overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            @forelse ($video->tracks as $i => $track)
                                <tr class="border-b border-gray-50 last:border-0 dark:border-gray-800/50">
                                    ...
                                    <td class="px-5 py-3 text-right text-theme-xs">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <button type="button"
                                                    @click="$store.confirm.ask('{{ route('admin.videos.tracks.destroy', [$video, $track]) }}', '{{ __('Videoni o‘chirishni tasdiqlaysizmi?') }}')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-5 py-8 text-center text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Video yo‘q') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div x-show="showStore" x-cloak
                    ...
                    <div class="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                        <div class="mb-5 flex items-center justify-between">
                            <h4 class="text-lg font-semibold text-gray-800 dark:text-white/90">{{ __('Yangi video') }}</h4>
                            <button type="button" @click="showStore = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">&times;</button>
                        </div>
                        <form method="POST" action="{{ route('admin.videos.tracks.store', $video) }}" enctype="multipart/form-data" class="space-y-4"
                              x-data="uploadForm({ chunked: { video_file: 'video' } })" @submit="submitUpload($event)">
                            @csrf
                            ...
                            <x-admin.form.file name="video_file" :label="__('Video fayl')" accept="video/*" with-progress :help="__('MP4/WEBM/MOV, 950 MB gacha')" />
                ...
                @foreach ($video->tracks as $track)
                    ...
                    <div x-show="editId === {{ $track->id }}" x-cloak
                        ...
                        <div class="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                            <div class="mb-5 flex items-center justify-between">
                                <h4 class="text-lg font-semibold text-gray-800 dark:text-white/90">{{ __('Videoni tahrirlash') }}</h4>
                                <button type="button" @click="editId = null" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">&times;</button>
                            </div>
                            <form method="POST" action="{{ route('admin.videos.tracks.update', [$video, $track]) }}" enctype="multipart/form-data" class="space-y-4"
                                  x-data="uploadForm({ chunked: { video_file: 'video' } })" @submit="submitUpload($event)">
                                @csrf @method('PUT')
                                ...
                                <x-admin.form.file name="video_file" :label="__('Video fayl')" accept="video/*" with-progress
