...
@section('title', __('Dissertatsiyalar'))

@section('content')
    {{-- Title + New dissertation --}}
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800 dark:text-white/90">{{ __('Dissertatsiyalar') }}</h2>
            <p class="text-theme-sm mt-1 text-gray-500 dark:text-gray-400">{{ __('Jami') }}: {{ $dissertations->total() }}</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.dissertations.export', $filters) }}"
            ...
            <a href="{{ route('admin.dissertations.create') }}"
               class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition">
                <span class="text-lg leading-none">+</span> {{ __('Yangi dissertatsiya') }}
            </a>
        </div>
    </div>
    ...
    {{-- Search / filter --}}
    <form method="GET" action="{{ route('admin.dissertations.index') }}"
        ...
        <div class="flex gap-2">
            ...
            @if (array_filter($filters))
                <a href="{{ route('admin.dissertations.index') }}" class="flex h-11 items-center rounded-lg border border-gray-200 px-4 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-800 dark:text-gray-400">{{ __('Tozalash') }}</a>
            @endif
        </div>
    </form>
                ...
                <tbody>
                    @forelse ($dissertations as $dissertation)
                        <tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-white/[0.02]">
                            <td class="px-5 py-4">
                                <div class="min-w-0">
                                    <p class="text-theme-sm truncate font-medium text-gray-800 dark:text-white/90">{{ $dissertation->title }}</p>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.dissertations.show', $dissertation) }}"
                                    ...
                                    <a href="{{ route('admin.dissertations.edit', $dissertation) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-12 text-center">
                                ...
                                <p class="mt-2 text-theme-sm text-gray-500 dark:text-gray-400">{{ __('Dissertatsiyalar topilmadi.') }}</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
